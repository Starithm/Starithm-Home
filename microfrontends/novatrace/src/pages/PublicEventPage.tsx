...
export default function PublicEventPage({ canonicalId }: { canonicalId?: string }) {
        ...
        <div style={{ marginLeft: isMobile ? '0' : 'auto', flexShrink: 0, display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          {!isSignedIn ? (
            <SignInButton mode="modal" forceRedirectUrl={window.location.href}>
              <button title="Sign in" onClick={saveReturnUrl} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#666', display: 'flex', alignItems: 'center', padding: '0.25rem' }}>
  ...
  note      = {Real-time astronomical event monitoring, Starithm}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', minWidth: 0 }}>
            ...
            <CommentThread canonicalId={event.canonicalId} compact />
